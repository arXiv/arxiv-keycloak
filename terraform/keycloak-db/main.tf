...
# Individual Secret Manager secrets for each SSL certificate
# This approach is cleaner than bundling in a shell script
# Each certificate is mounted as a separate file in the keycloak container

# Secret for server CA certificate (validates the database server)
resource "google_secret_manager_secret" "authdb_server_ca" {
  secret_id = "authdb-server-ca"
  project   = var.gcp_project_id
  ...
  labels = {
    database = var.instance_name
    purpose  = "ssl-server-ca"
  }
}

resource "google_secret_manager_secret_version" "authdb_server_ca" {
  secret      = google_secret_manager_secret.authdb_server_ca.id
  secret_data = google_sql_database_instance.auth_db.server_ca_cert[0].cert
}

# Secret for client certificate (authenticates the client to the server)
resource "google_secret_manager_secret" "authdb_client_cert" {
  secret_id = "authdb-client-cert"
  project   = var.gcp_project_id
  ...
  labels = {
    database = var.instance_name
    purpose  = "ssl-client-cert"
  }
}

resource "google_secret_manager_secret_version" "authdb_client_cert" {
  secret      = google_secret_manager_secret.authdb_client_cert.id
